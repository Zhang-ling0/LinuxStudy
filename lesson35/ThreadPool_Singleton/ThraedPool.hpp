...
namespace NS_THREAND_POOL
{
    ...
    public:
    //如果是多线程获取单例呢？切换每个都创建了一个对象，那么这个单例不就形同虚设了吗？这个只适用于单执行流中获取单例--加上锁
        // 对外提供接口，获取该单例对象
        // 类的成员函数，必须要有对象才能调用，但是对象是单例的
        // 多线程安全了，但是效率比较低，从第二次开始
        static ThreadPool<T> *Instance()
        {
            if(nullptr ==_instance);//双if判断，该条件不满足就直接退出了解决频繁申请无效锁
            {
            grouplock lockgroup(_lock);
            if (nullptr == _instance)
            {
                // 第一次调用
                _instance = new ThreadPool<T>();
                _instance->Start(); // 在类内调用
    ...
    private:
        ...
        Mutex _mutex;                 // 保护任务队列的锁--保证数据安全的锁
        Cond _cond;                   // 条件变量
        int _slaver_sleeper_count;    // 有多少slaver正在休眠

        // 添加单例模式--懒汉（饿汉就将*去掉）
        static ThreadPool<T> *_instance; // 类内定义静态指针，初始化一般放在类外
        static Mutex _lock;//保证单例安全的锁
    };
    template <typename T>
